...
def estimate_syll(word):
    ...
    if len(word_l) > 2 and word_l[-2:] == 'es':
        if (word_l[-3] not in VOWELS) and (word_l[-3] not in ['x', 'c', 's', 'z', 'h', 'g']):
            syll -= 1

    if len(word_l) > 2 and word_l[-2:] == 'ed':
        if (word_l[-3] not in VOWELS) and (word_l[-3] in ['n', 'm', 'k', 'g', 's', 'z', 'h', 'w']):
            syll -= 1
    ...
    for i in range(0, num_haiku):
        ...
        for x in [5, 7, 5]:
            ...
            while linesyl < x:
                ...
                options = []
                new_cur = ""
                # until you have some words to pick from which won't overload the syllable needs and are of a 
                # compatible next POS 
